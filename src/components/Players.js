		...
		<ul className='row players'>
    		{this.props.players.map(player =>
  				<li className='col-xs-2' key={player.id}>
						<div>{player.name}</div>
						<div className={!player.auction.isIn ? 'hidden': ''}> Points Auction (Punti Asta): {player.auction.points}</div>
						...
						<div className={(player.id !== this.props.auction.winner)? 'hidden': ''}>Winner Auction (Vincitore Asta)!</div>
